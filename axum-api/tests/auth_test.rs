...
use axum_api::models::{login_user::Claims, error::AuthError, user::UserRole};
use jsonwebtoken::{encode, Header, Algorithm, DecodingKey, EncodingKey};
...
async fn test_jwt_token() {
     ...
     let jwt_public_key = std::env::var("JWT_PUBLIC_KEY").expect("JWT_PRIVATE_KEY must be set in .env");
    ...
    let data = decode_jwt_token(&token_string, &jwt_decoding_key).expect("token should decode successfully");
