...
Dashmap is reliable due to its sharding nature compared to Hashmap i.e., 
you can concurrent read-write without global lock. broadcast::Sender 
enables fan-out — one driver location update is delivered to N customer 
subscribers watching that driver simultaneously. Tradeoff : broadcast 
drops messages for slow receivers when the buffer fills. For live 
location tracking, tokio::watch would be more correct since only the 
latest position matters — this is a known tradeoff I'd address before 
production.
